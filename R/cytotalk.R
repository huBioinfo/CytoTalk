...
run_cytotalk <- function(
    ...
    use_cache = TRUE, cache_dir = "C:/Users/Administrator/Desktop/temp_Data") {
    ...
    if (use_cache && file.exists(network_cache_file)) {
      ...
      #min_max_normalize
      mat_intra_a_norm<-min_max_normalize(Matrix::as.matrix(mat_intra_a))
      mat_intra_b_norm<-min_max_normalize(Matrix::as.matrix(mat_intra_b))
      
      # 如果启用了缓存，则保存计算结果
      if (use_cache) {
        if (echo) cat("Saving network construction results to cache...\n")
        save(mat_intra_a, mat_intra_b, mat_intra_a_norm, mat_intra_b_norm, file = network_cache_file)
      }
    }
